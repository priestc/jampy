...
To record desktop audio (virtual instruments, software synths, browser audio, etc.) with Takeloom, you need a virtual audio loopback device. This creates a fake audio input that captures whatever your system is playing.
...
Now anything playing through your system is simultaneously sent to BlackHole. In `takeloom studio-setup`, select **BlackHole 2ch** as one of your audio interfaces and label its input.
...
PipeWire provides monitor sources automatically. Run `takeloom studio-setup` and look for a device with "Monitor" in the name (e.g. "Monitor of Built-in Audio Analog Stereo"). Select it as an interface and label its input.
...
```bash
pactl load-module module-null-sink sink_name=TakeloomLoopback sink_properties=device.description="Takeloom Loopback"
```

Then route your application's audio to the "Takeloom Loopback" sink. The corresponding monitor source will appear as an input device in `takeloom studio-setup`.
...
3. In `takeloom studio-setup`, select **CABLE Output** as an audio interface and label its input

## Usage in Takeloom

Once your virtual loopback device is set up:

1. Run `takeloom studio-setup`
...
5. Record as normal with `takeloom start-session <instrument>`
